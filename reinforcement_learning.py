...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            return actions[self.rng.int(0, len(actions)-1)] #used self.rng to generate a random index for the actions list
        else:
            ...
            best_action = None # will be used to return best action based on best computed q-value
            for action in actions:
                q_value = self.compute_q(env, observation, action) #compute q value for all actions
                if q_value > max_q_value: # using strictly bigger than to ensure that the first best is returned
                    max_q_value = q_value
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        Old_q = self.Q[str(state)][str(action)] # store the old value
        if next_action == None: # if next state is terminal state update q value of next_state and next_action to zero
            self.Q[str(next_state)][str(next_action)] = 0
        # apply the update rule Q(s,a) = Q(s,a) + alpha*(r + gamma Q(s',a') - Q(s,a))
        self.Q[str(state)][str(action)] += self.learning_rate *(reward + self.discount_factor*self.Q[str(next_state)][str(next_action)]-Old_q)
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        # apply bellman's equation by finding max utility value returned by an action in the action list
        max_utility = -float('inf')
        for action in self.actions:
            Q_value = self.compute_q(env,state,action)
            max_utility = max(Q_value,max_utility)
        return max_utility
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        old_Q = self.Q[str(state)][str(action)] # store old q value
        if done : # handeling terminal next_state by stating  as zero
            next_state_utility = 0
        else:
            next_state_utility = self.compute_utility(env, next_state) # otherwise compute nest state as stated in equation
        # apply Q-learning update equation $Q(s,a)= Q(s,a) + alpha*(r + gamma*max_{a'}Q(s',a') - Q(s,a)) where max_{a'}Q(s',a') is computing the maximum utility of the next_state
        self.Q[str(state)][str(action)] += self.learning_rate*(reward + self.discount_factor*(next_state_utility) -old_Q)
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        for feature, value in features.items():
            # Q(s,a) = sum(w*x) # where w is the weight in the self.weights dictionary and value is the features of the state s
            q_value += self.weights[action][feature]*value
        return q_value
    ...
    def __compute_utility_from_features(self, features: Dict[str, float]) -> float:
        # applying bellman again to get maximum utility
        max_utility = -float('inf')
        for action in self.actions:
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        features = self.feature_extractor.extract_features(env, state)
        if done: # if next stsate is a terminal state change its q value to zero
            Q_next_state =0
        else: # if not terminal state then compute the utility of the next state to use in equation
            Q_next_state = self.__compute_utility_from_features(self.feature_extractor.extract_features(env,next_state))
        # store q_value of current state to insert in update equation and save computation
        Q_value = self.compute_q(env,state,action)
        for feature, value in features.items(): # loop on all features of current state and apply equation to update weights of each feature
            self.weights[action][feature]+= self.learning_rate*(reward + self.discount_factor*Q_next_state-Q_value)*value
